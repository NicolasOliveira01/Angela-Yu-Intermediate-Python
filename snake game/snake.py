...
STARTING_POSITIONS = [(0, 0), (-20, 0), (-40, 0)] # as posições iniciais da cobra no inicío do jogo, a cobra no começo tem 3 elementos
MOVE_DISTANCE = 20
RIGHT = 0
UP = 90
LEFT = 180
...
class Snake:
    ...
    def reset(self):
        for seg in self.segments: # manda a cobra antiga para eu lugar fora da tela
            seg.goto(1000, 1000)
        self.segments.clear() # remove todos os elementos da lista segments
        self.create_snake() # cria a cobra inicial
        self.head = self.segments[0]
        # está fazendo tudo o que está sendo feito no inicio porque vai inicializar (__init__) a cobra de novo
